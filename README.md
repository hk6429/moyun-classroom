...
3. 選中頁面後，可插入投票、選擇題、填空、開放問題、便利貼、文字雲或手寫互動。
...
在教材編輯按「在此頁後插入互動」，將活動類型選為「文字雲」。每位學生提交一個 30 字以內的詞語或短語，可修改回應；相同詞語合併計數，字越大代表人數越多。全半形與英文字母大小寫會合併。教師即時查看，按「公布回應／答案」後停止收件，學生可看匿名文字雲；重新開放後可繼續修改。原始作答可匯出。

### 十題競速測驗

首頁選「建立十題競速測驗」，會建立十題可編輯的國文暖身題。各題可設 5～300 秒，或一次套用全部十題。請確認題目、選項與正解後開課；全班先加入，老師按開始後倒數三秒，學生才會看見題目。每題只能送出一次，老師收卷或時間到才公布正解與累計排名，再依序開始下一題。

答對分數為 `max(0, 1000 - floor(作答毫秒 / (時限毫秒 × 0.9) × 1000))`；答錯或逾時為 0 分。最後 10% 的時間答對亦為 0 分；20 秒題目從第 18 秒起沒有分數。十題滿分 10,000 分，同總分並列名次。以伺服器收到請求的時間計分，裝置時間與客戶端指定分數無效；網路延遲仍可能影響分數。

第一題開始後暫停新成員加入；不可重做或倒回題目。結束後可看全班最終排名，匯出總排名及逐題成績 CSV（含分數與作答秒數）。中途結束時未作答題目計 0 分。
