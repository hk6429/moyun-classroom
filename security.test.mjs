...
test('教師登入、隔離權限、持久化、完整名冊與回饋',async()=>{
 ...
 await call('logout',{},null,'');assert.equal((await call('state?code='+h.code,null,h.token)).status,200);const cross=await fetch(base+'/api/create',{method:'POST',headers:{Origin:'https://evil.example',Cookie:cookie,'Content-Type':'application/json'},body:JSON.stringify({deck})});assert.equal(cross.status,403);process.env.ALLOWED_ORIGINS='https://classroom.pages.dev,https://classroom.netlify.app';const allowed=await fetch(base+'/api/create',{method:'POST',headers:{Origin:'https://classroom.netlify.app',Cookie:cookie,'Content-Type':'application/json'},body:JSON.stringify({deck})});delete process.env.ALLOWED_ORIGINS;assert.equal(allowed.status,200);await call('logout',{});assert.equal((await call('state?code='+h.code,null,h.token)).status,401);
